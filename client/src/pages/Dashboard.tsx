...
const Dashboard = () => {
  ...
  const dailyCalorieLimit = user?.dailyCalorieIntake || 2000
  const today = new Date().toISOString().split('T')[0]

  const todayFood = useMemo(
    () => allFoodLogs.filter((f: FoodEntry) => f.createdAt?.split('T')[0] === today),
    [allFoodLogs, today]
  )

  const todayActivities = useMemo(
    () => allActivityLogs.filter((a: ActivityEntry) => a.createdAt?.split('T')[0] === today),
  ...
  const motivation = getMotivationalMessage(totalCalories, totalActiveMinutes, dailyCalorieLimit)
      ...
      <div className='dashboard-grid'>
        <Card className='shadow-lg col-span-2'>
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='text-right'>
              ...
              <p className='text-2xl font-bold text-slate-800 dark:text-white'>{dailyCalorieLimit}</p>
            </div>
          </div>
          <ProgressBar value={totalCalories} max={dailyCalorieLimit} />
          ...
          <div className='mt-4 flex justify-between items-center'>
            ...
            <span className='text-sm text-slate-400'>{Math.round((totalCalories / dailyCalorieLimit) * 100)}%</span>
          </div>
        ...
        <div className='dashboard-card-grid'>
          ...
          <Card>
            ...
            <p className='text-sm text-slate-500'>activities logged</p>
          </Card>
        </div>
                  ...
                  <div className='flex justify-between items-center mb-2'>
                    ...
                    {(() => {
                      ...
                      const getStatus = (b: number) => {
                        ...
                        if (b < 30) return { color: 'text-yellow-500' }
